...
*****************************************************************************/
//向服务器上传标签状态
U8 upload_hpr_state_to_server(int mode)
{
    cJSON *root = NULL;
    cJSON *data_obj = NULL;
	...
	if(mode == RTK_MODE)
	{
		cJSON_AddNumberToObject(data_obj, "latitude", NMEA_Data_ptr.lat);
		cJSON_AddNumberToObject(data_obj, "longitude", NMEA_Data_ptr.lon);
		cJSON_AddNumberToObject(data_obj, "altitude", NMEA_Data_ptr.alt);
		cJSON_AddNumberToObject(data_obj, "mode", server_Data.mode);	
	}
	else if(mode == UWB_MODE)
	{
		cJSON_AddNumberToObject(data_obj, "latitude", g_pos_info.tag_position[0]);
		cJSON_AddNumberToObject(data_obj, "longitude", g_pos_info.tag_position[1]);
		cJSON_AddNumberToObject(data_obj, "altitude", g_pos_info.tag_position[2]);
		cJSON_AddNumberToObject(data_obj, "mode", server_Data.mode);//UWB模式用数值 7 代表	
	}
	else
	{
		cJSON_AddNumberToObject(data_obj, "latitude", 0);
		cJSON_AddNumberToObject(data_obj, "longitude", 0);
		cJSON_AddNumberToObject(data_obj, "altitude", 0);
		cJSON_AddNumberToObject(data_obj, "mode", 0);
	}
...
*****************************************************************************/
U8 UDP_upload_hpr_location(int mode)
{
	cJSON *root = NULL;
	...
	if(mode == RTK_MODE)
	{
		cJSON_AddNumberToObject(root, "lat", NMEA_Data_ptr.lat);
    	cJSON_AddNumberToObject(root, "lon", NMEA_Data_ptr.lon);
	}
	else if(mode == UWB_MODE)
	{
		cJSON_AddNumberToObject(root, "lat", g_pos_info.tag_position[0]);
    	cJSON_AddNumberToObject(root, "lon", g_pos_info.tag_position[1]);
	}
	else
	{
...
{	
	...
	while(1)
	{
		...
		if(SERVER_RTK == POS_INVALID && g_pos_info.tag_position_valid_flag == POS_INVALID)
		{
			server_Data.mode = 0;
			if(loop_cnt%100 == 0)
			{
				...
				DBG_SERVER_Print("\r\n设备定位失效!!!\r\n\r\n");
				DBG_SERVER_Print("-------------------------------------------\r\n");
				//GLOBAL_PRINT(("Sending UDP!!\r\n"));
				if(!MQTT_OFF_LINE)
				{
					upload_hpr_state_to_server(DATA_ERROR);
				}
				UDP_upload_hpr_location(DATA_ERROR);

			}
			PRINT_FLAG = FALSE;
		}
		else if(SERVER_RTK==POS_VALUE_VALID||g_pos_info.tag_position_valid_flag==POS_VALUE_VALID)
		{
			...
			if(PRINT_FLAG == TRUE)
			{
				if(!MQTT_OFF_LINE)
				{
					upload_hpr_state_to_server(UPLOAD_VAL);
				}
				delay_ms(5);
				UDP_upload_hpr_location(UPLOAD_VAL);
				
				DBG_SERVER_Print("-------------------------------------------\r\n");
				DBG_SERVER_Print("\r\nHPR_OUTPUT:[UWB],%d,%lf,%lf,%lf,%d,%lf,%d,%lf,[RTK],%d,%lf,%lf,%lf,[MODE],%d\r\n\r\n",\
							Pos_time.UWB_TIME,\
