...
#if 1
	GLOBAL_HEX(dw1000local.deviceID);
...
#if 0
	//DIS_STXP 设置为1，配置为手动功率模式
	reg = dwt_read32bitreg(SYS_CFG_ID);
	reg |= (1<<18); 
	dwt_write32bitreg(SYS_CFG_ID,reg);

	reg = dwt_read32bitreg(SYS_CFG_ID);
	reg &= ~(TX_POWER_TXPOWPHR_MASK | TX_POWER_TXPOWSD_MASK); 
	//dwt_write32bitreg(TX_POWER_ID,reg);
	dwt_write32bitreg(TX_POWER_ID, 0x00c0c000);
#endif

		 dwt_write32bitreg(TX_POWER_ID, 0x1f1f1f1f);
	//	dwt_write32bitreg(TX_POWER_ID, 0xc0c0c0c0);
//	dwt_write32bitreg(TX_POWER_ID, 0);
    ...
    GLOBAL_HEX(dw1000local.partID);
    GLOBAL_HEX(dw1000local.lotID);
...
uint32 dwt_read32bitoffsetreg(int regFileID,int regOffset)
{
	...
	GLOBAL_MEMSET(buffer,0x0,sizeof(uint8)*4);

    result = dwt_readfromdevice(regFileID,regOffset,4,buffer); // Read 4 bytes (32-bits) register into buffer
