...
static void _iotclient_monitor_thread(void* arg)
{
    ...
    while(1)
    {
		...
		while((iotRTCMDev.sta != IOT_STA_IDEL)&&(iotDev.sta == IOT_STA_IDEL))
		{
			...
			{
				case IOT_STA_INIT:
					iotRTCMDev.bMqttSub = FALSE;
					iotRTCMDev.bMqttConnect = FALSE;
					iotRTCMDev.sta = IOT_STA_MQTTCONNECT;
					break;
					
				 case IOT_STA_MQTTCONNECT:
	            
	                if(!iotRTCMDev.bMqttConnect)
	                {//未连接到MQTT服务器
	                    retry_cnt++;
	                    connectMqttServer(&iotRTCMDev);
	                    if(retry_cnt>3)
	                    {
	                        retry_cnt = 0;
	                        iotDev.sta = IOT_STA_INIT;
